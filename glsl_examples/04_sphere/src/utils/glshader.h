...
public:
  ...
  virtual bool SetupUniforms(const GLDrawData &draw_data) const;
  virtual bool SetLights(const glm::mat4 &view_matrix,
                         const std::vector<std::shared_ptr<Light>> &lights) const;
  virtual bool SetMaterial(std::shared_ptr<Material> material) const;
...
//! \brief GLPhongShader class
class GLPhongShader : public GLShader {
public:
  using Ptr = std::shared_ptr<GLPhongShader>;
  using WeakPtr = std::weak_ptr<GLPhongShader>;
  bool SetLights(const glm::mat4 &view_matrix,
                 const std::vector<std::shared_ptr<Light>> &lights) const override;
  bool SetMaterial(std::shared_ptr<Material> material) const override;
};
